...
This plan synthesizes the consultant's architectural spec with deep knowledge of the existing codebase (Vite 7 + React 19 + R3F + drei + GSAP + Howler.js + existing UniversePage patterns).
...
```js
{
  ...
  type: enum,                       // core|hub|epoch|project|moment|thought|milestone|person|place|tag|track
...
{
  id: string,
  type: enum,                       // chronological|geographic|collaboration|client|thematic|family|influence|simultaneous|growth
  visibility: enum,
  from: nodeId,
  to: nodeId,
...
- Outputs `constellation.graph.json`, `constellation.layout.json`, `constellation.narration.json`
...
- `UniversePage.jsx:129-222` InteractiveNode patterns (orbit, hover scale, GSAP camera fly-to, navigate)
- `UniversePage.jsx:281-285` PostProcessing setup (Bloom + ChromaticAberration + Vignette)
- `sounds.js` playHoverSound, playClickSound

**Acceptance**: Renders 150+ nodes in instanced batches, edges visible, hover tooltip works, click focuses camera
...
**Acceptance**: Panel shows media/text/tags, clicking a chip highlights matching nodes, meaning lens shows evidence
...
**Acceptance**: Scrub smoothly moves camera along helix, epoch labels appear, triggers narration events

### Ticket 07: Narrator Overlay + Scripted Engine
**Files**:
- `src/components/constellation/NarrationOverlay.jsx`
- `src/utils/narratorEngine.js` (event-driven: ENTER_EPOCH, FOCUS_NODE, HOVER_EDGE, DISCOVERY_MILESTONE, IDLE, TOUR_STEP)
...
**Selection logic**: recentNarrations queue, relevance scoring, novelty weighting, emotional pacing, per-tier cooldowns

**Acceptance**: Narration fires correctly per event, no repetition, typewriter animation, dismissible
...
**Reuse**: existing Web Audio oscillator pattern from `sounds.js:18-32`, Howler.js analyser from `AudioContext.jsx`
...
Parse:
- `your_instagram_activity/media/posts_1.html` → posts with captions, dates, GPS, tagged users
...
- Optionally: `your_instagram_activity/comments/post_comments_1.html`, `likes/liked_posts.html`

**Output**: `data/normalized/instagram.json`
**Acceptance**: Posts become moment nodes, tagged users become person entities, GPS creates place entities with city-level precision
...
- `projects/*/project.json` (35 files with id, name, description, role, client, date, type, texts, imageUrls, videoStreams)
- `about/about.json` (career history, awards, exhibitions, education, clients, specialties)
- `blog/blog.json` (20 posts)

**Acceptance**: 35 projects + 20 blog posts + 11 milestones appear as nodes, client/collaborator edges work

### Ticket 12: Discovery System + XP + Constellation Modes
**Files**: Update ConstellationScene + ConstellationPage

**Reuse**: existing `GameOverlay.jsx` XP system, existing `localStorage` discovery pattern from `UniversePage.jsx:226-237`
...
- **Constellation modes**: "Life" / "Work" / "Ideas" (changes filters + edge weights + narration tone)
...
1. **Path memory**: Faint glowing trail showing the visitor's journey through the constellation
...
4. **Constellation modes**: Life (warmth-forward) / Work (project-forward) / Ideas (topic-forward)
5. **2D library fallback**: Searchable node index for accessibility + "real website" credibility
6. **Trail suggestion**: "Follow this thread →" with next node glow when exploring connections
...
```
src/App.jsx                     -- Add /constellation route (lazy-loaded ConstellationPage)
src/utils/sounds.js             -- Add discover/focus/connection/narration/timeline sounds
...
UniversePage.jsx:92-127         -- CoreNode (wireframe icosahedron + transmission material)
UniversePage.jsx:129-222        -- InteractiveNode (orbit, hover, GSAP fly-to, navigate)
UniversePage.jsx:281-285        -- PostProcessing (Bloom + ChromaticAberration + Vignette)
...
3. **Scene**: `/constellation` renders 150+ nodes at 60fps, instanced batches, connections visible
4. **Interaction**: Hover → tooltip + edge highlight. Click → fly-to + detail panel + narration
...
10. **No regressions**: All existing pages (Home, Workshop, Vault, Garden, Now, etc.) unaffected

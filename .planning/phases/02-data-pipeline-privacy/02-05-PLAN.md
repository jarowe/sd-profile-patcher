...
must_haves:
  ...
  key_links:
    - from: "src/pages/Admin.jsx"
      to: "public/data/pipeline-status.json"
      via: "fetches pipeline run status (lastRun, status, stats) for display"
      pattern: "pipeline-status\\.json"
    - from: "src/pages/Admin.jsx"
      to: "curation.json"
      via: "reads hidden list and visibility_overrides for toggle state"
...
<task type="auto">
  ...
  <action>
    ...
    - If the pipeline FAILS at any stage (parse error, privacy audit failure, schema validation failure):
      ...
      - Write failure status to public/data/pipeline-status.json: { lastRun: ISO timestamp, status: "failed", error: "description of failure" }
    ...
    - If the pipeline SUCCEEDS:
      ...
      - Write success status to public/data/pipeline-status.json with full stats
    - IMPORTANT: pipeline-status.json is the ONLY file the pipeline writes for status tracking. curation.json is READ-ONLY input to the pipeline (managed by admin UI only).
  ...
  <verify>
    ...
    Verify pipeline-status.json has lastRun and stats after run (NOT curation.json).
    Verify curation.json is UNCHANGED by pipeline execution.
    ...
    Test resilience: Run pipeline, verify output exists, then cause a failure (e.g., corrupt a required file temporarily), run pipeline again, verify original output files are NOT overwritten and pipeline-status.json shows failure.
  </verify>
  <done>
    Pipeline preserves last good output on failure, writes status to pipeline-status.json (not curation.json). .gitignore excludes generated data and private exports. Admin route is lazy-loaded at /admin. .env.example documents required environment variables.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    2. **Pipeline status section** (read-only):
       - Fetch `public/data/pipeline-status.json` at mount (this is where the pipeline writes its status)
       - Also fetch curation.json for the hidden/override state
       - Display:
         - Last run: timestamp (from pipeline-status.json)
       ...
       - If pipeline-status.json not found, show "Pipeline has not been run yet" message
       - If curation.json not found, show defaults (no hidden nodes, no overrides)
    ...
    4. **Publish/hide toggle behavior**:
       ...
       - IMPORTANT: The downloaded curation.json should contain ONLY the curation fields (hidden, visibility_overrides) -- NOT pipeline status fields. Pipeline status lives in pipeline-status.json.
  ...
  <verify>
    ...
    Verify pipeline status is read from pipeline-status.json (not curation.json).
    Verify publish/hide toggle updates local state.
    Verify "Save Changes" generates a downloadable curation.json with ONLY hidden[] and visibility_overrides (no pipeline status fields).
  ...
  <done>
    Thin admin page at /admin shows pipeline run status from pipeline-status.json and a metadata table with publish/hide toggles. Curation.json is read for toggle state and downloaded (not server-written) for updates. Auth gate uses env-var key. Handles missing data gracefully. Styled with existing glass-panel aesthetic.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    2. Pipeline writes status to pipeline-status.json (curation.json is read-only input)
    3. Privacy audit fails the build if any violations detected
    4. Thin admin at /admin shows pipeline status and publish/hide controls
    5. Pipeline preserves last good output on failure
  </what-built>
  <how-to-verify>
    ...
    3. Check `public/data/pipeline-status.json` -- verify it has lastRun, status, and stats
    4. Verify curation.json was NOT modified by the pipeline run
    5. Verify no private content in output: search for visibility "private" in the JSON
    6. Visit `http://localhost:5173/admin` -- enter admin key, verify pipeline status shows (reading from pipeline-status.json), node table displays
    7. Toggle a node's publish/hide, click Save, verify curation.json download contains only hidden[] and visibility_overrides
    8. Visit `http://localhost:5173/constellation` -- verify existing 3D scene still works with mock data (frontend migration not done yet)
    9. Run `npm run build` -- verify build succeeds (pipeline runs as prebuild)
  </how-to-verify>
...
- Pipeline status section reads from pipeline-status.json (NOT curation.json)
- Node table displays all nodes from pipeline output
- Publish/hide toggle works and Save generates valid curation.json (without pipeline status fields)
...
<success_criteria>
Thin admin page provides read-only pipeline status (from pipeline-status.json) and basic publish/hide controls (modifying curation.json via download). Pipeline resilience preserves last good output. Clean separation: curation.json = read-only input (version-controlled), pipeline-status.json = runtime output (in public/data/, gitignored). The full Phase 2 pipeline is operational: parse real data, enforce privacy, generate edges, emit deterministic JSON, and fail-closed on any privacy violation.
</success_criteria>

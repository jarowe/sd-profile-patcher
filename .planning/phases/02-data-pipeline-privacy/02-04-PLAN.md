...
files_modified:
  ...
  - curation.json
autonomous: true
requirements: [PRIV-02, PRIV-05, PRIV-06, PRIV-07, PRIV-08]
...
must_haves:
  ...
  artifacts:
    - path: "pipeline/privacy/visibility.mjs"
      provides: "Visibility tier assignment and allowlist enforcement"
    ...
    - path: "curation.json"
      provides: "Node publish/hide state"
  key_links:
...
<objective>
Implement fail-closed privacy validation, visibility tier enforcement, minors policy, and allowlist system.

Purpose: Privacy is the critical safety net for the entire pipeline. This plan ensures that no private content leaks into public output. The build MUST fail (exit code 1) if any privacy violation is detected. This is the "belt and suspenders" layer -- even if parsers make mistakes, the privacy audit catches it.
...
<task type="auto">
  <name>Task 1: Visibility tiers, allowlist, and minors guard</name>
  <files>
    pipeline/privacy/visibility.mjs
    pipeline/privacy/minors-guard.mjs
    allowlist.json
  ...
  <action>
    ...
    Create `curation.json` (version-controlled, initial template):
    ```json
    {
      "_comment": "Node curation state. Pipeline reads this to apply publish/hide. Managed via admin UI.",
      "hidden": [],
    ...
    - `last_pipeline_run` and `pipeline_stats` are written by the pipeline for admin display

    Create `pipeline/privacy/visibility.mjs`:
    - Export `assignVisibility(node, allowlist, curationOverrides)`:
      - Start with node's default visibility (from parser: "private" for Instagram, "public" for Carbonmade)
      - Apply curation override if exists for this node ID
      - Apply allowlist precedence (LOCKED: most restrictive rule wins):
        ...
        - If explicitly on curation override -> use that
      - Return final visibility tier string
  ...
  <done>
    Visibility assignment enforces most-restrictive-wins rule. Allowlist controls public name visibility. Minors guard strips last names, GPS, and blocked patterns. Curation.json template ready for admin UI. All privacy utilities export correctly.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - After parse phase: Apply visibility assignment to all nodes (assignVisibility)
    ...
    - Update curation.json with last_pipeline_run timestamp and pipeline_stats (node counts, edge counts, violation count).
    - Pipeline exit codes: 0 = success, 1 = privacy violation or schema error, 2 = no data (empty pipeline)

    The privacy audit MUST be the LAST step before declaring success. No writing should occur after the audit passes except the stats update to curation.json.

    Actual pipeline execution order after this plan:
    1. Parse (Instagram + Carbonmade)
    2. Assign visibility tiers
  ...
  <verify>
    ...
    Verify curation.json is updated with last_pipeline_run timestamp after successful run.
...
- curation.json updated with pipeline stats after run
...
<success_criteria>
Privacy audit provides fail-closed guarantee. Build exits 1 on any leak. Visibility tiers enforced with most-restrictive-wins. Minors protected per PRIV-05 (first names only, no GPS/school/home). People visible publicly only via explicit allowlist. DMs and contact graphs blocked. Schema validation catches structural issues.
</success_criteria>

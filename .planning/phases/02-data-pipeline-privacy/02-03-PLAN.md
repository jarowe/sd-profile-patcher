...
must_haves:
  truths:
    - "Running `node pipeline/index.mjs` produces constellation.graph.json and constellation.layout.json in public/data/"
...
<task type="auto">
  ...
  <action>
    ...
    3. **Curation phase** (read curation.json if exists):
       - Load curation.json from PIPELINE_CONFIG.curation.file
  ...
  <verify>
    ...
    Verify output files exist: `ls public/data/constellation.graph.json public/data/constellation.layout.json`
    ...
    Verify determinism: Run `node pipeline/index.mjs` twice and diff outputs -- should be identical.
  ...
  <done>
    Pipeline runs end-to-end producing constellation.graph.json and constellation.layout.json from real data. Edge generation uses locked signal weights with evidence arrays. Output is deterministic (byte-identical across runs). Frontend data loader is ready for component migration. Pipeline runs via `npm run pipeline` and automatically before `npm run build`.
  </done>
</task>
...
- Running pipeline twice produces identical output (diff returns no differences)
- `npm run pipeline` works as a script command
</verification>

<success_criteria>
Complete pipeline produces valid constellation JSON from real Carbonmade data (and Instagram if available). Evidence-based edges use the locked signal weight table. Output is deterministic and matches the mock-constellation.json schema for drop-in replacement. Frontend data loader bridges the gap between pipeline output and component consumption.
</success_criteria>

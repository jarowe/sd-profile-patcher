...
must_haves:
  truths:
    - "User can hover any node to see a 3D billboard label showing the node title, type, and date (REND-02)"
...
- Drei `<Billboard follow={true}>` containing multiple `<Text>` children:
  - Title line: `<Text fontSize={1.2} color="white" anchorX="center" anchorY="bottom" outlineWidth={0.05} outlineColor="black" position={[0, 0.6, 0]}>`
  - Type + date line: `<Text fontSize={0.7} color={nodeTypeColor} anchorX="center" anchorY="bottom" outlineWidth={0.03} outlineColor="black" position={[0, 0, 0]}>` showing e.g. "project | 2023-04"
- Display title, type, and date per REND-02 requirement ("tooltip with title, type, and date")
- Type text uses the node's type color (amber for project, coral for moment, etc.)
...
- Add CSS for toolbar and scrubber positioning
  ...
  <done>
    - Hovering a node shows 3D billboard text label with node title, type, and date (REND-02)
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. **Hover test**: Move mouse over nodes -- 3D text labels should appear at each node showing its title

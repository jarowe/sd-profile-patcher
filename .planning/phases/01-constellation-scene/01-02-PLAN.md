...
must_haves:
  truths:
    - "User can hover any node to see a 3D billboard label showing the node title"
...
- Drei `<Billboard follow={true}>` containing `<Text fontSize={1.2} color="white" anchorX="center" anchorY="bottom" outlineWidth={0.05} outlineColor="black">`
- Display node title only (per locked decision: "Title only -- clean, fast")
...
- Subscribe to `focusedNodeId` from store. When it changes to a non-null ID:
...
- Add CSS for toolbar and scrubber positioning
  ...
  <done>
    - Hovering a node shows 3D billboard text label with node title

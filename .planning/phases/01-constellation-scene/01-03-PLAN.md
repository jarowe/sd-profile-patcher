...
- Ensure ConnectionLines renders BEHIND NodeCloud in the scene (add before NodeCloud in JSX order, or use `renderOrder`)
  ...
  <verify>
    <automated>cd C:/dev/jarowe && npx vite build 2>&1 | tail -5 && echo "--- Evidence check ---" && node -e "const fs=require('fs'); const dp=fs.readFileSync('src/constellation/ui/DetailPanel.jsx','utf8'); const ok=dp.includes('evidence'); console.log('DetailPanel has evidence section:', ok); if(!ok) process.exit(1);" && node -e "const fs=require('fs'); const cl=fs.readFileSync('src/constellation/scene/ConnectionLines.jsx','utf8'); const ok=cl.includes('focusedNodeId') && cl.includes('useConstellationStore'); console.log('ConnectionLines subscribes to focusedNodeId:', ok); if(!ok) process.exit(1);"</automated>
  </verify>
  <done>
    ...
    - DetailPanel.jsx contains evidence rendering code (verified by grep)
    - Build succeeds
  </done>
</task>
